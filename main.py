...
def testMannWhitney(data1, data2):
    """
    Функция проводит для двух выборок тесты Шапиро и возвращает их p-значения
    :param data1: первая выборка pd.DataFrame
    :param data2: вторая выборка pd.DataFrame
    ...
    _markdownPval("Результаты теста Манна-Уитни для выборок:", results.pvalue)
    
    
def testStudent(data1, data2):
    """
    ...
    _markdownSinglePval("Результаты теста Стьюдента для выборок:", results.pvalue)    
...
def renderHypothesis(hypothesis, work_days, age=0, alpha=0.05):
    ...
    if hypothesis == "2 Гипотеза":
        st.markdown(f'### Формулировка гипотез:   \
            \n1. Null Hypothesis $H_0$ - Старшие сотрудники (> {age} лет) пропускают в течение года более {work_days} рабочих дней по болезни **реже или сопоставимо** их более молодым коллегам (<= {age} лет).   \
    ...
    if hypothesis == "1 Гипотеза":
        st.markdown(f'### Формулировка гипотез:   \
            \n1. Null Hypothesis $H_0$ - Мужчины пропускают в течение года более {work_days} рабочих дней по болезни **реже или сопоставимо** женщинам.   \
...
def renderConclusion(hypothesis, test, testResult, work_days, age=0):
    ...
    if hypothesis == "1 Гипотеза":
        if testResult:
            st.markdown(f'### Вывод:   \
                \n Тест {test_string} дал нам основания отвергнуть нулевую гипотезу, и мы имеем права сказать, что:   
            ...
            st.markdown(f'### Вывод:   \
                ...
                \n**Мужчины пропускают в течение года более {work_days} рабочих дней по болезни **реже или сопоставимо** женщинам.**')
    if hypothesis == "2 Гипотеза":
        if testResult:
            st.markdown(f'### Вывод:   \
                \n Тест {test_string} дал нам основания отвергнуть нулевую гипотезу, и мы имеем права сказать, что:   
            ...
            st.markdown(f'### Вывод:   \
                \n Тест {test_string} не дал нам оснований отвергнуть нулевую гипотезу, вывод:   
                \n**Старшие сотрудники (> {age} лет) пропускают в течение года более {work_days} рабочих дней по болезни **реже или сопоставимо** их более молодым коллегам (<= {age} лет).**')
...
def main():    
    ...
    if not df.empty:
        ...
        if selected_hypothesis == "1 Гипотеза":
            ...
            renderHypothesis("1 Гипотеза", work_days=work_days)
        else:
            age = st.slider('Возраст', min_value=min_age, max_value=max_age)
            work_days = st.slider('Кол-во пропущенных дней', min_value=min_days, max_value=max_days)
            renderHypothesis("2 Гипотеза", work_days=work_days, age=age)
        ...
        if start_button:
            ...
            if (testShapiro(df_1, df_2)):
                ...
                test_result = testStudent(df_1, df_2)
                ...
                test_result = testMannWhitney(df_1, df_2)
